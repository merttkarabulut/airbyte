...
from source_shiftbase.streams import IncrementalShiftbaseStream, Absentees, Availabilities
...
def test_get_updated_state_preserves_newer(patch_incremental_base_class, mock_accounts, mock_start_date):
    ...
    current_state = {
        "test_account": {"updated": "2024-07-01T10:00:00Z"}
    }
    latest_record = {
        "account_name": "test_account",
...
def test_request_params_with_state(patch_incremental_base_class, mock_accounts, mock_start_date):
    ...
    stream_state = {
        "test_account": {"updated": "2024-06-01T00:00:00Z"}
    }
