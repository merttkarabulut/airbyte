    ...
    def check_connection(self, logger, config) -> Tuple[bool, Any]:
        ...
        try:
            ...
            for account in accounts:
                ...
                headers = {
                    "Authorization": f"API {access_token}",
                ...
                if response.status_code != 200:
                    return False, f"Failed to connect to account {account_name} with status code: {response.status_code}, response: {response.text}"
            ...
            for department in departments_stream.read_records(sync_mode=None):
                if isinstance(department, dict):
                    dept_data = {
                        ...
                        'account_name': department.get('account_name')
                    }
